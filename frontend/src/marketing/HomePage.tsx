...
// ── Flow animation between hero cards ────────────────────────────────────────
function FlowArrow() {
  return (
    <div className="flex items-center flex-shrink-0 w-12 md:w-20">
      <svg viewBox="0 0 80 20" className="w-full h-5 overflow-visible">
        ...
        {/* Static dashed track */}
        <path d="M 0 10 H 70" stroke="#1e3a5f" strokeWidth="1.5" strokeDasharray="4 4" fill="none" />
        {/* Animated flow */}
        <path
          d="M 0 10 H 70"
        ...
        />
        {/* Arrowhead */}
        <polygon points="70,6 80,10 70,14" fill="#60a5fa" opacity="0.8" />
      </svg>
    </div>
  )
}
...
function HeroFlow() {
  ...
  return (
    <div className="relative flex flex-col md:flex-row items-center justify-center gap-2 md:gap-0 mt-12 md:mt-16 px-4">
      {/* Glow backdrop */}
      <div className="absolute inset-0 bg-gradient-to-r from-blue-600/5 via-purple-600/10 to-blue-600/5 rounded-3xl blur-2xl pointer-events-none" />

      {/* ── Card 1: Trial Balance ── */}
      <div
        className="glass rounded-2xl p-4 w-56 flex-shrink-0 animate-float-up glow-blue-sm z-10"
        style={{ animationDelay: "0s" }}
      >
        <div className="flex items-center gap-2 mb-3">
          <FileSpreadsheet className="w-4 h-4 text-blue-400" />
          <span className="text-xs font-semibold text-slate-300">Trial Balance</span>
        </div>
        <div className="text-[10px] text-slate-500 mb-2">Dec 2024 vs Nov 2024</div>
        {[
          ...
          <div key={row.name} className="flex items-center justify-between py-1 border-b border-slate-800/60 last:border-0">
            <span className="text-[10px] text-slate-400">{row.name}</span>
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] font-mono text-slate-300">{row.cur}</span>
      ...
      {/* ── Card 2: AI Engine ── */}
      <div className="glass rounded-2xl p-5 w-44 flex-shrink-0 z-10 animate-glow-pulse">
        <div className="flex flex-col items-center gap-3">
          <div className="relative">
            <div className="w-12 h-12 rounded-full bg-blue-600/20 flex items-center justify-center animate-glow-pulse">
          ...
          <div className="text-center">
            <div className="text-xs font-semibold text-slate-300 mb-0.5">AI Analysis</div>
            <div className="text-[10px] text-slate-500">claude-sonnet-4-6</div>
          </div>
          <div className="w-full">
            <div className="flex justify-between text-[9px] text-slate-500 mb-1">
              <span>Processing</span>
              <span className="text-blue-400">247 accounts</span>
            </div>
      ...
      <div
        className="glass rounded-2xl p-4 w-56 flex-shrink-0 animate-float-down glow-blue-sm z-10"
        style={{ animationDelay: "2s" }}
      >
        <div className="flex items-center gap-2 mb-3">
          <TrendingUp className="w-4 h-4 text-fav" />
          <span className="text-xs font-semibold text-slate-300">Flux Commentary</span>
        ...
        <div className="text-[10px] text-slate-300 leading-relaxed min-h-[60px]">
          {typed}
          <span className="inline-block w-0.5 h-3 bg-blue-400 ml-0.5 animate-type-cursor align-middle" />
        </div>
        <div className="mt-3 flex items-center gap-1.5">
          <div className="w-1.5 h-1.5 rounded-full bg-fav animate-pulse" />
          <span className="text-[9px] text-slate-500">AI confidence: High</span>
        </div>
      </div>
    </div>
  )
...
function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? "glass border-b border-slate-800/60 py-3" : "py-5"}`}>
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-bold text-white text-lg tracking-tight">Nordavix</span>
        </div>
        <div className="hidden md:flex items-center gap-8">
          {[
            ...
            <a key={item.label} href={item.href} className="text-sm text-slate-400 hover:text-white transition-colors">
              {item.label}
            </a>
          ))}
        </div>
        <div className="flex items-center gap-3">
          <Link to="/app" className="hidden md:block text-sm text-slate-400 hover:text-white transition-colors px-4 py-2">
            Sign in
          </Link>
          <Link
            to="/app"
...
function HeroSection() {
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden pt-20 pb-12 px-6">
      ...
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-blue-600/20 rounded-full blur-[120px] animate-blob pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-purple-600/20 rounded-full blur-[100px] animate-blob-delay pointer-events-none" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-indigo-600/10 rounded-full blur-[80px] animate-blob pointer-events-none" style={{ animationDelay: "8s" }} />

      {/* Content */}
      <div className="relative z-10 text-center max-w-4xl mx-auto">
        {/* Badge */}
        <div className="inline-flex items-center gap-2 glass px-4 py-1.5 rounded-full text-xs font-medium text-blue-400 mb-6 animate-fade-in-up">
        ...
        {/* Headline */}
        <h1 className="text-5xl md:text-7xl font-bold text-white leading-[1.05] tracking-tight mb-6 animate-fade-in-up animation-delay-200">
          Month-end close,{" "}
          <span className="text-gradient">automated.</span>
        </h1>

        {/* Subheadline */}
        <p className="text-lg md:text-xl text-slate-400 max-w-2xl mx-auto leading-relaxed mb-8 animate-fade-in-up animation-delay-400">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-4 animate-fade-in-up animation-delay-700">
          <Link
            to="/app"
            className="group flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold px-8 py-3.5 rounded-xl transition-all duration-200 glow-blue text-base"
          >
            Start for free
          ...
          <a
            href="#how-it-works"
            className="flex items-center gap-2 glass hover:bg-slate-800/60 text-slate-300 hover:text-white font-medium px-8 py-3.5 rounded-xl transition-all duration-200 text-base"
          >
            See how it works
          </a>
        </div>
        ...
        <p className="text-xs text-slate-600 animate-fade-in-up animation-delay-1000">
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-slate-600 animate-bounce">
        <div className="w-5 h-8 border border-slate-700 rounded-full flex items-start justify-center pt-1.5">
          <div className="w-1 h-2 bg-slate-600 rounded-full animate-bounce animation-delay-200" />
        </div>
      </div>
    </section>
  )
...
function StatsBar() {
  ...
  return (
    <section ref={ref} className="border-y border-slate-800/60 bg-slate-900/40 py-10 px-6">
    ...
    <section id="features" className="py-24 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 text-blue-400 text-xs font-semibold uppercase tracking-widest mb-4">
        ...
        >
          {FEATURES.map((f, i) => {
            ...
            return (
              <div
                key={f.title}
                className={`glass-light rounded-2xl p-6 hover:border-slate-600/60 transition-all duration-500 group ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
...
function HowItWorksSection() {
  ...
  return (
    <section id="how-it-works" className="py-24 px-6 bg-slate-900/30">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 text-purple-400 text-xs font-semibold uppercase tracking-widest mb-4">
        ...
        <div ref={ref} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative">
          ...
          <div className="hidden lg:block absolute top-10 left-[12.5%] right-[12.5%] h-px bg-gradient-to-r from-blue-600/0 via-blue-600/40 to-blue-600/0" />
          ...
          {STEPS.map((step, i) => {
            ...
            return (
              ...
              >
                {/* Number badge */}
                <div className={`relative w-20 h-20 rounded-2xl bg-gradient-to-br ${step.color} flex items-center justify-center mb-5 shadow-lg z-10`}>
                  <Icon className="w-8 h-8 text-white" />
                  <div className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-slate-950 border border-slate-700 flex items-center justify-center text-[10px] font-bold text-slate-400">
                    {step.num.slice(-1)}
                  </div>
                </div>
                ...
                {[
                  {
                    ...
                    pvar: "11.6%",
                    status: "Approved",
                    statusColor: "text-fav bg-fav/10",
                    material: true,
                    fav: true,
                    narr: "Revenue increased $256K (+12%) vs prior period, driven by strong Q4 seasonal demand in the enterprise segment...",
                  },
                  {
                    acc: "6100 · COGS",
                    cur: "1,124,000",
                    ...
                    material: true,
                    fav: false,
                    narr: "COGS increased $92K (+9%) consistent with revenue growth. Gross margin remained stable at 54.2%...",
                  },
                  {
                    acc: "7200 · Salaries",
                    cur: "542,000",
                    ...
                    material: true,
                    fav: false,
                    narr: "Salaries increased $32K due to Q4 bonus accruals and two new hires in the engineering team...",
                  },
                  {
                    acc: "5200 · Other Revenue",
                    cur: "12,400",
                    pri: "0",
                    dvar: "12,400",
                    pvar: "N/M",
                    ...
                    material: false,
                    fav: true,
                    narr: "New account — no prior period balance. One-time consulting revenue. Review with FP&A.",
                  },
                ].map((row, i) => (
                  <tr key={i} className={`border-b border-slate-800/40 hover:bg-slate-800/30 transition-colors ${row.material ? "font-medium" : ""}`}>
  ...
  return (
    <section className="py-24 px-6 bg-slate-900/30">
      <div className="max-w-4xl mx-auto">
        ...
        <div
          ref={ref}
          className={`glass rounded-2xl overflow-hidden transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        >
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800">
                <th className="text-left px-6 py-4 text-slate-400 font-medium w-1/2">Capability</th>
                <th className="text-center px-4 py-4 text-slate-400 font-medium">Manual</th>
                <th className="text-center px-4 py-4 text-slate-400 font-medium">Checklist tools</th>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.feature} className="border-b border-slate-800/40 hover:bg-slate-800/20 transition-colors">
                  ...
                  {[row.manual, row.checklist, row.nordavix].map((val, j) => (
                    <td key={j} className="text-center px-4 py-3">
                      {val
                        ? <CheckCircle2 className={`w-4 h-4 mx-auto ${j === 2 ? "text-blue-400" : "text-slate-500"}`} />
    ...
    <section id="pricing" className="py-24 px-6">
      ...
      >
        <div className="relative glass rounded-3xl p-12 overflow-hidden">
          {/* Background glow */}
          <div className="absolute inset-0 bg-gradient-to-br from-blue-600/10 via-purple-600/10 to-blue-600/5 pointer-events-none" />
          <div className="absolute -top-20 -right-20 w-60 h-60 bg-blue-600/15 rounded-full blur-3xl pointer-events-none" />

          <div className="relative z-10">
            <div className="inline-flex items-center gap-1.5 text-xs font-medium text-yellow-400 bg-yellow-400/10 px-3 py-1 rounded-full mb-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link
                to="/app"
                className="group flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold px-10 py-4 rounded-xl transition-all duration-200 glow-blue text-base"
              >
                Get started free
                <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </Link>
