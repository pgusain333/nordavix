...
def _build_cover_sheet(
    ...
    ws["A2"] = "NORDAVIX"
    ws["A2"].font = ws["A2"].font.copy(name="Calibri", size=10, bold=True, color="3E8F66")

    ws["A4"] = "Month-end close package"
    ws["A4"].style = "nx_h1"
    ...
    rows = [
        ("PERIOD ENDING",  fmt_date(period_end)),
        ("GENERATED",      fmt_date(datetime.utcnow().date()) + " · " + datetime.utcnow().strftime("%I:%M %p UTC")),
        ("GENERATED BY",   generated_by or "(unknown user)"),
        ("FILE CONTENTS",  "Reconciliations, Prepaids, Accruals, Fixed Assets, Leases, Loans, Audit log"),
    ]
